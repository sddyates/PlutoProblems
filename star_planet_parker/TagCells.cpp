...
/* ----------------------------------------------------------------
  ...
  /* -----------------------------------------------
     ...
     /* - Modification to tell pluto to refine the area around star and planet.. - */
     double sep, Rp, Rs, rad_p, rad_s;
     sep = g_inputParam[seperation]*1.49597892e+11/UNIT_LENGTH;
     Rp = 1.0*g_inputParam[planet_radius]*0.10045*CONST_Rsun/UNIT_LENGTH;
     Rs = 1.0*g_inputParam[star_radius]*CONST_Rsun/UNIT_LENGTH;
     rad_p = EXPAND((x1-sep)*(x1-sep), + x2*x2, +x3*x3);
     rad_p = sqrt(rad_p);
